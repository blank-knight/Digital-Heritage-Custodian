...
contract DigitalHeritageMVP is ReentrancyGuard, Ownable {
    ...
    modifier onlyConfigured() {
        require(
            heritageConfigs[msg.sender].isConfigured,
            "Heritage not configured"
        );
        _;
    }
    ...
    function updateActivity() external onlyConfigured {
        require(
            !heritageConfigs[msg.sender].isInheritanceTriggered,
            "Inheritance already triggered"
        );
    ...
    function transferTokenToHeir(
        ...
        require(config.isInheritanceTriggered, "Inheritance not triggered");
        require(
            validatorConfirmations[msg.sender] >= REQUIRED_CONFIRMATIONS,
     ...
     * @dev 获取验证者确认状态
     */
    function getValidatorConfirmation(
        address owner,
        address validator
